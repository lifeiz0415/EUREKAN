...
- Chrome 계열의 MP3 재생에서는 `<audio>.currentTime`을 기준으로 재생 위치를 갱신하되, 사용자가 플레이바를 드래그하는 동안에는 선택 위치를 임시 보존하고 드래그가 끝난 뒤 한 번만 실제 seek를 커밋한다. Safari의 Web Speech API 우회에서는 `SpeechSynthesisUtterance` 경계 이벤트를 우선 사용해 재생 위치를 갱신한다.
