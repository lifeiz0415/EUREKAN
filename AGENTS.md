...
- 홈 상단 인기 주제 제목에는 분야별 최신 대표글 수를 두 자리 숫자로 함께 표시한다.
...
- 홈의 분야별 섹션 제목과 분야별 전체 보기 제목에는 해당 분야의 전체 페이지 수를 두 자리 숫자로 함께 표시한다.
...
## agents 폴더 규칙
- `agents/`는 운영 문서용으로만 유지한다.
- 실제 런타임 코드와 연결되지 않아도 된다.
