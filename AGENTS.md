...
- 한국주식·미국주식·크립토 글의 대표 이미지에서 종목·코인 식별성이 중요할 때는 공식 로고나 프로젝트 로고를 작은 배지처럼 보조 요소로 넣을 수 있다. 단, AI가 로고를 임의로 그리게 두지 말고 정확한 형태의 벡터·공식 자산·명확히 재현 가능한 단순 배지를 합성하며, 캡션과 `credit`에 로고 출처를 함께 표기한다.
