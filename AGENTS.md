...
## 이미지 규칙
- `images/` 폴더는 생성형 이미지 또는 직접 관리하는 공개용 이미지 자산 저장용으로 유지한다.
- 이미지가 꼭 필요한 정적 글은 웹검색으로 주제와 맞는 외부 이미지 URL을 찾거나, 직접 생성·관리하는 이미지 파일을 `images/` 안에 저장해 상대 경로로 연결한다.
...
- 이미지 생성 작업은 품질을 유지하면서 속도를 높이기 위해 아래 `고품질 빠른 모드`를 기본 플로우로 삼는다.
- `고품질 빠른 모드`는 글 1개마다 `제목·slug 확인 → pages/<slug>.md 본문 핵심 3줄 추출 → 첫 본문 문단과 소제목 확인 → 이미지 장면 한 문장 확정 → 톤 프리셋 선택 → 이미지 생성 또는 외부 이미지 선택 → WebP 변환 → app.js/정적 HTML 메타데이터 치환 → 빠른 검증 → 커밋·푸시` 순서로 처리한다.
...
- `AI 느낌이 없어야 한다`는 요구가 있으면 먼저 공개 라이선스 실제 사진 후보를 검토하고, 글의 핵심 장면과 직접 맞는 후보가 없을 때만 사실적 사진 프롬프트로 생성한다.
- 생성 이미지는 기본적으로 2:1 카드 썸네일에 맞게 WebP로 저장하고, `images/<slug>.webp`처럼 slug 기반 파일명을 사용한다. 같은 이미지를 다른 글에 재사용하지 않는다.
- 한국주식·미국주식·크립토 글의 대표 이미지에서 종목·코인 식별성이 중요할 때는 공식 로고나 프로젝트 로고를 작은 배지처럼 보조 요소로 넣을 수 있다. 단, AI가 로고를 임의로 그리게 두지 말고 공식 PNG/WebP 같은 래스터 로고 자산을 우선 사용해 합성하며, SVG를 직접 그려 로고를 재현하지 않는다. 캡션과 `credit`에는 로고 출처를 함께 표기한다.
- 속도를 위해 이미지 프롬프트는 매번 처음부터 길게 새로 만들지 않고, 선택한 톤 프리셋에 글별 핵심 장면, 대상, 장소, 피해야 할 요소만 갈아 끼워 작성한다.
- 빠른 검증은 최소 `node --check app.js`, 정적 HTML JSON-LD 파싱, `git diff --check`, WebP 파일 형식 확인을 포함한다. 화면 깨짐이 의심되거나 레이아웃·이미지 로딩을 바꾼 경우에는 브라우저 검증까지 추가한다.
...
- `app.js` 매니페스트에는 `thumbnail` 메타데이터를 넣지 않고, 이미지가 필요한 경우 외부 URL 또는 `images/` 상대 경로를 담은 `image` 객체만 사용한다.
