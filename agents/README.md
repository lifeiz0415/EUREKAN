# Yggdrasil Agent Personas

이 폴더는 Yggdrasil에서 사용되는 에이전트 페르소나 문서 모음이다.

## 목적
- 각 에이전트의 역할, 입력, 출력, 책임, 금지 사항을 분리해 문서화한다.
- 기능이 늘어나면 같은 규칙으로 새로운 에이전트 문서를 계속 추가할 수 있다.
- 관리자는 이 문서들을 기준으로 각 에이전트 작업 결과를 승인/반려한다.
...
## 조직 구조 원칙
- Yggdrasil의 에이전트는 기능 조직과 주제별 전문 데스크 조직이 겹쳐 있는 매트릭스 구조를 따른다.
- 기능 조직은 탐색, 수집, 생성, 연결, 검증, 유지보수, 실험, 승인 흐름을 담당한다.
- 전문 데스크 조직은 특정 분야 문맥을 깊게 이해하고, 일반 에이전트의 판단을 보강한다.
- 모든 AI 에이전트의 결과는 최종적으로 `admin-guardian.md`가 상징하는 인간 승인권자에게 올라간다.
