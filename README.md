...
- 필요한 글의 외부 이미지 URL 기반 카드 썸네일과 검색엔진용 이미지 메타데이터 제공
- 상세 페이지 음성 읽기 버튼
...
5. 글에 이미지가 꼭 필요하면 저작권과 접근성을 확인한 외부 이미지 URL을 `app.js` 매니페스트의 `image` 객체와 `pages/<slug>.html`의 본문 이미지, Open Graph, Twitter 카드, JSON-LD에 직접 반영합니다. 화면에 보이는 카드와 본문 이미지는 가로 2:세로 1 비율로 표시합니다.
